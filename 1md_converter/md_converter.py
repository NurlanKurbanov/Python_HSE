...
def define_old_md_content(old_file_name):
    if not pathlib.Path.exists(pathlib.Path(".") / old_file_name):
        old_md_content = None
    else:
        old_md_content = read_data(old_file_name)
        if is_empty(old_md_content):
            old_md_content = None
    return old_md_content
...
def main():
    task_file_name = "2_4.py"
    old_file_name = "collection.md"

    content = read_data(task_file_name)
    old_md_content = define_old_md_content(old_file_name)
